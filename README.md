...
**给 DeepSeek Harness（DSH）网页引擎配的一个 Windows 桌面"守护壳"：改动前自动存档、出事一键回滚，上游破坏性更新或插件改坏配置都能退回原样；本地就能寻找、安装、管理插件，不需要另装插件市场；引擎一键启动与停止，日志随时可看、还能导出诊断——全程图形界面，不需要命令行。**

它面向不熟悉命令行的用户：安装后双击启动，点按钮即可运行引擎；出现故障、卡顿或插件状态异常时，也能在同一个窗口里查看和处理。

![快照页：改动前自动存档，出问题整组回滚](docs/screenshots/04-快照与回滚.png)

![插件页：本地就能寻找、安装与管理插件，不需要另装插件市场](docs/screenshots/05-插件-本地管理.png)

![日志页：日志随时可看，一键导出诊断](docs/screenshots/03-日志与诊断.png)
...
| 反复遇到的不便 | 守护壳里的对应做法 |
| --- | --- |
| 装了新插件或改了配置之后出了问题，没有还原点可退 | 快照页的改动前自动存档与一键回滚，出事就退回上一次正常的状态，参见[快照页](#三快照页改动前存档出事回滚) |
| 装了一堆插件，说不清谁在启用、谁被关掉、谁该升级；想装新的又没有可浏览的入口 | 插件页的本地清单与社区收录，寻找、安装、管理都在本机完成，不需要另装插件市场，参见[插件页](#四插件页本地插件与寻找插件) |
| 引擎起不来时没有任何现场，不知道去哪看日志 | 日志页的实时输出、历史日志与「导出诊断」，参见[日志页](#二日志页看清引擎说了什么) |
| 引擎按上游标签安装，上游一发破坏性更新，跑起来的就是另一个版本 | 版本记忆锁住现在这个版本，配合快照回滚，参见[设置页](#五设置页常规路径版本) |
...
- 按名称、作者、说明搜索，并可按下拉里的**排序**字段排列：**安装时间**（默认，从新到旧）、**创建日期**、**更新日期**、**兼容性**；同一项再点一次即反向，选中行右侧显示 `↑` / `↓`。
- 兼容性按「不兼容 → 基本可用 → 完全兼容 → 未声明」的固定顺序排列，**不随排序方向翻转**。
- 每张卡片显示插件名、作者、当前版本与启用状态，以及一段说明；名字后是**创建时间**（作者首次发版的时间），右上角是**本机的更新日期**，格式都是 `xxxx年xx月xx日`。
- 每张卡片可就地**「禁用插件」/「启用插件」**与**「卸载」**。
- 顶部「一键更新 N 个」把所有确有新版本的插件一次更新完；**有没有新版就看这里的计数**；更新前自动打快照兜底。
- 更新过程全程有时限（25 分钟），「取消下载」在全过程都可点；进度窗**不再强制置顶**，可以正常切出去做别的事。
- 「刷新」重新扫描本地插件并查询新版本。
...
- 更新或卸载插件前会先识别引擎状态：只有确实有会话在活动时才提醒；引擎空闲或未运行时不会打扰，提醒也不是禁止。
- 卸载后会同时核对包目录与插件清单，两处都清掉才算卸载完成；只清掉一部分时会如实报为**未卸干净**，并提示下次安装会把它装回来。
- 卡片上标注的兼容档来自插件自己声明的引擎版本要求；作者没有声明时显示为未声明，只能实测。也可以按**兼容性**排序，一眼看出哪些插件与当前引擎版本合得来。
...
- 点开截图缩略图可在看图界面里细看：**Ctrl + 滚轮**以光标为锚点缩放，**按住鼠标拖动**查看局部（位移小算点击、位移大算拖动）；左右两侧悬停浮出半透明箭头翻页（左右各两成宽度，图片外的空白也算），**点中间打开仓库页面**。
...
- 部分插件来自 Git 仓库，安装这类插件需要本机已安装 Git，**本程序不代为安装 Git**；其余大多数插件来自 npm 源，只要本机具备运行环境即可安装。
...
- 「守护壳版本」一栏检查**本程序自身**有没有新版本，与上面引擎的版本互不影响；查到新版时多出「立即更新」，可在本程序内直接下载并安装。
...
- Git（只有安装来源为 Git 仓库的插件才需要，**不是必须项**）。这类插件在安装时由系统的 Git 取回内容，本机没有 Git 就会安装失败；大多数插件来自 npm 源，用不到 Git。**本程序不代为安装 Git**
- 首次启动引擎时需要联网（引擎通过 npx 拉取并缓存）

用现成安装包的话，这些都不用手动准备；自己编译才需要 .NET 10 SDK。

**界面语言**：程序界面目前只有中文，没有多语言支持，系统的显示语言不影响界面文字。需要英文界面的话，欢迎到本仓库提交 Issue 提出，或直接提交改动参与贡献；需求较多时会补上英文界面。文档另有英文入口，见 [README.en.md](README.en.md)。
...
`Config` 目录下的 `plugin-times.json` 是守护壳自己的**本地记账**：记的是每个插件的**安装时间**与**本机的更新日期**，只在本机使用、不会对外发送，也可以随时删除。它从 1.3 开始记录，此前已装的插件显示为「未知」；卸载后重装则记最近一次。
...
若失败只出现在某一个插件上，多半是该插件来自 Git 仓库，而本机尚未安装 Git：这类插件在安装时由系统的 Git 取回内容，缺少 Git 便会失败。装好 Git 后重新安装即可；失败的具体原因可在日志页查看。
...
遇到问题或有改进想法，欢迎提交 **Issue** 或 **Pull Request**；文档纠错、使用体验上的建议同样欢迎，不一定非要写代码。

- 改动、验证与提交的具体流程见 [CONTRIBUTING.md](CONTRIBUTING.md)。
- 需要：.NET 10 SDK、Inno Setup 6（打包安装程序用）。

如果这个工具对你有帮助，欢迎点一个 **Star** —— 那是对作者最直接的鼓励，也能让更多需要它的人看到这个项目。
